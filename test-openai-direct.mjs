  ...
  try {
    ...
    const response = await callOpenAI({
      ...
      messages: [
        {
          role: 'user',
          content: 'What is 2+2? Respond with just the number in JSON format like {"answer": 4}'
        }
      ],
      response_format: { type: 'json_object' },
      max_tokens: 50
    ...
    const structuredResponse = await callOpenAI({
      ...
      messages: [
        {
          role: 'user',
          content: 'Create a simple math problem with its solution.'
        }
      ],
      response_format: {
        type: 'json_schema',
...
async function runAllTests() {
  ...
  await testWithO3Model();
  
  console.log('\n' + '='.repeat(50));
  console.log('✅ All tests completed!');
  console.log('\n📝 Summary:');
  console.log('- JSON Mode (json_object): Working with gpt-4o-mini');
  console.log('- Structured Outputs (json_schema): Working with strict schema compliance');
  console.log('- Predicted Outputs: Working with performance improvements');
  console.log('- o3 models: Not yet publicly available');
  console.log('\n✨ All OpenAI Response Format features are properly implemented!');
}
